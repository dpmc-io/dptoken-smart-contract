...
contract DPToken is ERC20, Ownable, Pausable, ReentrancyGuard {
    ...
    event UniswapDebug(address indexed sender, address indexed from, address indexed to, uint256 value);

    constructor() ERC20("DP Token", "DPT") {
        BUY_TAX = 10000; // 1%
        receiver = 0x129d92CEe2Fa6c3C5b177d877fD1E4C9731992F4;
        taxAddress = 0x69fCD7C8157738e2E95EAad0FDb09C61D432d958;
        /**
    ...
    // Buy Tax Percentage
    function updateBuyTaxPercentage(uint24 newBuyTaxPercentage) public onlyOwner {
        require(newBuyTaxPercentage <= 1000000, "Tax percentage must be 1000000 (100%) or less");
    ...
    // Universal Router
    function updateUniversalRouter(address newUniversalRouter) public onlyOwner {
        require(newUniversalRouter != address(0), "Universal router address cannot be zero");
    ...
    function updateSwapRouter02(address newSwapRouter02) public onlyOwner {
        require(newSwapRouter02 != address(0), "Swap router 02 address cannot be zero");
    ...
    function updateSwapRouter(address newSwapRouter) public onlyOwner {
        require(newSwapRouter != address(0), "Swap router address cannot be zero");
    ...
    function isWhitelistedOrUniswapAdress(address _address) public view returns (bool) {
    ...
    function transfer(address _to, uint256 _value) public override nonReentrant returns (bool) {
        ...
        require(!isBlacklisted(msg.sender), "T1 - Error: Sender's address is blacklisted and cannot initiate transactions.");
        require(!isBlacklisted(_to), "T2 - Error: Recipient's address is blacklisted, and the transaction cannot be completed.");
        ...
        if (!isWhitelistedOrUniswapAdress(msg.sender)) {
            require(!paused(), "T3 - Transaction failed: The sender's address is not whitelisted, and the contract is currently paused.");
        }

        // Check if the recipient is whitelisted or a UniswapV3Pool address
        if (!isWhitelistedOrUniswapAdress(_to)) {
            require(!paused(), "T4 - Transaction failed: The recipient's address is not whitelisted, and the contract is currently paused.");
        }
    ...
    function transferFrom(address _from, address _to, uint256 _value) public override whenNotPaused nonReentrant returns (bool success) {
        // Check for blacklisted addresses
        require(!isBlacklisted(_from), "TF1 - Error: Sender's address is blacklisted and cannot initiate transactions.");
        require(!isBlacklisted(_to), "TF2 - Error: Recipient's address is blacklisted, and the transaction cannot be completed.");

        // Check if the contract is paused for non-whitelisted and non-UniswapV3Pool addresses
        if (!isWhitelistedOrUniswapAdress(_from)) {
            require(!paused(), "TF3 - Transaction failed: The sender's address is not whitelisted, and the contract is currently paused.");
        }

        // Check if the recipient is whitelisted or a UniswapV3Pool address
        if (!isWhitelistedOrUniswapAdress(_to)) {
            require(!paused(), "TF4 - Transaction failed: The recipient's address is not whitelisted, and the contract is currently paused.");
        }
